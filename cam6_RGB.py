...
def searchfaces(num_faces):
    ...
    global face_detected, detection_counter, no_detection_counter, new_face, is_chroma_selected
...
def get_BPM():
    ...
    RED = [avs[0] for avs in avs_RGB] 
    GREEN = [avs[1] for avs in avs_RGB] 
    BLUE = [avs[2] for avs in avs_RGB] 

    #red = butter_bandpass_filter(RED, lowcut, highcut, fps, 5)
    #green = butter_bandpass_filter(GREEN, lowcut, highcut, fps, 5)
    #blue = butter_bandpass_filter(BLUE, lowcut, highcut, fps, 5)

    axis = np.arange(len(RED))
    x = axis/fps #time axis
    plt.figure(figsize=(10, 6))

    plt.plot(x, RED, label='Red Channel', color='red', marker='o')
    plt.plot(x, GREEN, label='Green Channel', color='green', marker='o')
    plt.plot(x, BLUE, label='Blue Channel', color='blue', marker='o')
    # Adding titles and labels
    plt.title('filtered RGB Channel Values')
    plt.xlabel('Time [s]')
...
signal_length = int(6*fps) # length for 6 seconds
