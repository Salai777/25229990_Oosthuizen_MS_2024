...
def get_BPM():
    global avs_RGB, fps, BPM, SMOOTH_BPM
    ...
    padded_HR = np.zeros(256) #zero padding
    padded_HR[:len(windowed_HR)] = windowed_HR

    fft = np.fft.fft(padded_HR)
    frequencies = np.fft.fftfreq(len(padded_HR), 1.0/fps)

    fft_pos = fft[:len(fft)//2] #take positive comonents
    frequencies_pos = frequencies[:len(frequencies)//2]

    max_power_index = np.argmax(fft_pos)
    dominant_frequency = frequencies_pos[max_power_index]
...
try:
    ...
    while(True):
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            smooth = np.zeros(len(BPM))
            smooth[-len(SMOOTH_BPM):] = SMOOTH_BPM

            # Time axis
            axis = np.arange(len(BPM))
            x = axis / 2  # Assuming each index corresponds to 0.5 seconds

            # Mask out zero values in smooth
            non_zero_mask = smooth != 0
            x_non_zero = x[non_zero_mask]
            smooth_non_zero = smooth[non_zero_mask]

            # Plotting
            plt.figure(figsize=(10, 6))

            # Plot only non-zero values of smooth and full BPM data
            plt.plot(x_non_zero, smooth_non_zero, color='blue', marker='o', label='SMOOTH_BPM')
            plt.plot(x, BPM, color='red', marker='o', label='BPM')

            plt.title('BPM Estimation using the Chrominance Method')
            plt.xlabel('Time [s]')
            plt.ylabel('BPM')
