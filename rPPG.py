...
def get_BPM():
    ...
    padded_HR = np.zeros(1024) #zero padding
...
b, a = butter(5, [lowcut / (0.5 * fps), highcut / (0.5 * fps)], btype='band', analog=False)
...
try:
    ...
    while(True):
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
                axis = np.arange(len(SMOOTH_BPM))
                x = axis/2 #time axis
                plt.figure(figsize=(10, 6))

                plt.plot(x, SMOOTH_BPM, color='blue', marker='o')
